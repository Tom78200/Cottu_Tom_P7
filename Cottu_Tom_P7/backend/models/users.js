...
class User {
    ...
    signup(userSql) {
        ...
        return new Promise((resolve, error) => { //L'objet Promise (pour « promesse ») est utilisé pour réaliser des traitements de façon asynchrone. Une promesse représente une valeur qui peut être disponible maintenant, dans le futur voire jamais.
        ...
        return new Promise((resolve, error) => {
            mysql_con.query(recupEmail, function(err, result) {
                ...
                if (list[0] == "") { console.log("Utilisateur non inscrit."); }
                    ...
                    bcrypt.compare(password, result[0].password) // Récupère le password dans la row que la base de donnée a envoyé
                    .then(valid => {
                        console.log("Vérification mot de passe...");
                        if (!valid) return error({ message: "Le mot de passe est incorrect " })
                        resolve({
                            userId: result[0].id, 
                            token: jwt.sign({  // creation du token avec l'user id
                                userId: result[0].id,
    ...
    modifyUser(userDataName, userDataEmail, userDataPassword, userDataId) {
        const email = userDataEmail[0]; let name = userDataName[0]; let password = userDataPassword[0]; let userId = userDataId[0];
        ...
        .then(hash => {
            ...
            return new Promise((resolve, reject) => {
                mysql_con.query(modifyUser, function(err, result) {
                    ...
                    resolve({ message : "Le profil a été modifié " })
                    reject({ message : "Modification non actée" })
                })
            })
        })
    }

    // Recevoir les données du profil 
    getProfil(dataUserId) {
        ...
        return new Promise((resolve, reject) => {
            mysql_con.query(getDataUser, function(err, result) {
                if (err) return reject({ error : "Impossible de récupérer les informations utilisateur" });
                resolve(result);
            }) 
